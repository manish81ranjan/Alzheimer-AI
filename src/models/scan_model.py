...
def create_scan(doc: dict):
    ...
    result = mongo.db.scans.insert_one(payload)
...
def get_raw_scan_by_id(user_id: str, scan_id: str):
    ...
    return mongo.db.scans.find_one({
        "_id": oid,
        "userId": user_id
    })
...
def delete_scan_by_id(user_id: str, scan_id: str):
    ...
    result = mongo.db.scans.delete_one({
        "_id": oid,
        "userId": user_id
    })
...
def update_prediction(user_id: str, scan_id: str, pred: dict):
    oid = _to_objectid(scan_id)
