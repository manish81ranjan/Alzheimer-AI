...
#     x = preprocess_image(image_path=image_path, img_size=Config.IMG_SIZE)
#     y = model.predict(x, verbose=0)
...
#         raise ValueError(
#             f"Model output shape mismatch. Expected {num_classes} values, got {y.shape[0]}"
#         )
...
#         probs_arr = softmax(y)
#     else:
#         probs_arr = y.astype(np.float32)

#     labels = [class_map[str(i)] for i in range(num_classes)]
...
def run_prediction(image_path: str, model_path: str) -> dict:
    """
    Run real model inference and return normalized prediction output.
    ...
    # If output is not already probabilities, apply softmax
    s = float(np.sum(y))
    if not (0.98 <= s <= 1.02):
        probs_arr = softmax(y)
    else:
    ...
    # Build ordered labels from class_map
    labels = [class_map[str(i)] for i in range(num_classes)]
    probs = {label: float(prob) for label, prob in zip(labels, probs_arr)}
